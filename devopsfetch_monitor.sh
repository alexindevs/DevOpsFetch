...
monitor_user_activities() {
    tail -fn0 /var/log/auth.log | while read line; do
        ...
        # SSH session opened
            ...
            log_message "User Activity" "SSH session opened for user $user"
        # SSH session closed
        elif echo "$line" | grep -q "sshd.*pam_unix(sshd:session).*session closed"; then
            user=$(echo "$line" | awk '{print $(NF-3)}')
            log_message "User Activity" "SSH session closed for user $user"
        fi
    done &
}
